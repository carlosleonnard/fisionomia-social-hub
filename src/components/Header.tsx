...
import { MobileMenu } from "./MobileMenu";              // Menu mobile responsivo
...
export const Header = () => {
  ...
  return (
    <header className="fixed top-0 z-50 w-full border-b border-border/50 bg-card/95 backdrop-blur supports-[backdrop-filter]:bg-card/80">
      <div className="px-2 md:px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-6 lg:w-80 lg:justify-center">
          <MobileMenu />
        ...
        <div className="flex-1 max-w-2xl mx-2 md:mx-8 lg:mr-4">
          <div className="relative" ref={searchRef}>
            <Search className="absolute left-3 md:left-4 top-1/2 transform -translate-y-1/2 text-muted-foreground h-3 w-3 md:h-4 md:w-4" />
            <Input 
              placeholder="Search famous people..." 
              className="pl-9 md:pl-12 h-9 md:h-12 bg-muted/30 border-border/30 focus:border-primary/50 rounded-full text-sm md:text-base"
              value={searchTerm}
            ...
            <Button 
              size="sm" 
              className="absolute right-1 md:right-2 top-1/2 transform -translate-y-1/2 bg-gradient-primary hover:shadow-button rounded-full h-7 md:h-8 px-2 md:px-3 text-xs md:text-sm"
              onClick={() => searchTerm && handleSearch(searchTerm)}
            >
              <span className="hidden md:inline">Search</span>
              <Search className="h-3 w-3 md:hidden" />
            </Button>
        ...
        <div className="flex items-center gap-1 md:gap-3">
          ...
          {user ? (
            ...
            <Button 
              variant="default" 
              className="bg-phindex-dark hover:bg-phindex-teal transition-all duration-300 rounded-full px-2 md:px-6 h-8 md:h-10 text-xs md:text-sm"
              onClick={() => setIsLoginModalOpen(true)}
              disabled={loading}
            >
              <svg className="mr-1 md:mr-2 h-3 w-3 md:h-4 md:w-4" viewBox="0 0 24 24">
